...
impl Dialog {
    pub fn from_bytes(in_bytes: &[u8]) -> Dialog {
        ...
        for _i in 0..bottom_size {
            ...
            let i_string = BKString {
                cmd,
                string: in_bytes[offset + 2..offset + 2 + str_size as usize].to_vec(),
            };
    ...
    pub fn read(path: &Path) -> Dialog {
        ...
        let bottom: Vec<BKString> = bottom_obj.iter().map(|y| BKString::from_yaml(y)).collect();

        let top_obj = doc["top"].as_vec().unwrap();
        let top: Vec<BKString> = top_obj.iter().map(|y| BKString::from_yaml(y)).collect();
...
impl QuizQuestion {
    ...
    pub fn read(path: &Path) -> QuizQuestion {
        ...
        let q: Vec<BKString> = q_obj.iter().map(|y| BKString::from_yaml(y)).collect();

        let a_obj = doc["options"].as_vec().unwrap();
        let a: Vec<BKString> = a_obj.iter().map(|y| BKString::from_yaml(y)).collect();
...
fn string_to_vecu8(string: &str) -> Vec<u8> {
    ...
    let mut squig_indx: Vec<usize> = string
        .windows(2)
        .enumerate()
...
impl DemoButtonFile {
    pub fn from_bytes(in_bytes: &[u8]) -> DemoButtonFile {
        ...
        let f1f = in_bytes[9];
        let inputs: Vec<ContInput> = in_bytes[4..]
            .chunks_exact(6)
            .map(|a| ContInput {
                x: a[0] as i8,
                y: a[1] as i8,
    ...
    pub fn read(path: &Path) -> DemoButtonFile {
        ...
        let inputs: Vec<ContInput> = inputs_yaml
            .iter()
            .map(|y| ContInput::from_yaml(y))
            .collect();
        DemoButtonFile {
            inputs,
            frame1_flag: f1f,
...
impl Asset for DemoButtonFile {
    fn to_bytes(&self) -> Vec<u8> {
        ...
        let mut input_bytes: Vec<u8> = self.inputs.iter().flat_map(|i| i.to_bytes()).collect();
    ...
    fn write(&self, path: &Path) {
        ...
        for input in self.inputs.iter() {
            writeln!(
                demo_file,
                "  - {{x: {:3}, y: {:3}, buttons: 0x{:04X}, frames: {}}}",
                input.x, input.y, input.buttons, input.frames
            )
            .unwrap();
        }
    }
...
impl Texture {
    ...
    pub fn to_rgba32(&self) -> Vec<u8> {
        match self.texture_type {
            ...
            _ => {
                panic!("Image type not implemented yet");
            }
        }
    }
    ...
    pub fn ci8_to_rgba32(ci8: &[u8], palatte: &[u8]) -> Vec<u8> {
        ...
        return ci8.iter().flat_map(|indx| pal[*indx as usize]).collect();
    }
...
impl SpriteFrame {
    pub fn new(bin: &[u8], file_offset: usize, format: &ImgFmt) -> SpriteFrame {
        ...
        SpriteFrame {
            ...
            chk_hdrs,
            palette: pal,
            pixel_data: pxl_data.into_iter().flatten().flatten().collect(),
        }
    }
}
    ...
    pub fn from_bytes(in_bytes: &[u8]) -> Sprite {
        ...
        if frame_cnt > 0x100 {
            ...
            let frame = SpriteFrame {
                ...
                h: chunk.h,
                header: Vec::new(),
                chk_hdrs: vec![in_bytes[8..16].to_vec()],
                palette: None,
